"""Llama-3-ELYZA-JP-8B (Q4_K_M GGUF) の単体動作確認 (Phase 3.7)。

RTX 5090 への GPU offload 可否、モデルロード時間、VRAM使用量、生成速度、
チャットテンプレートの自動検出、日本語出力の健全性を確認する。
Qwen/Swallow/LLM-jp用の chat_format を流用せず、chat_format=None (GGUF埋め込み
テンプレート = Llama 3 Instruct形式を自動使用) で検証する。
"""
