"""Phase 4ZH: Structural Identity Invariance Hardening 用データセット構築。

既存Phase4ZG candidate(1193件)にPhase4ZH新規教師(82件、うち10件はmulti-turn)を
追加した phase4zh_structural_hardened candidate を構築する。

既存1193件は無改変・読み取り専用で読み込み、新規82件のみを追加した別ファイルとして
新candidateを構築する。既存riru_phase4zg_identity_hardened_candidate.jsonl等は無改変。

品質検査: placeholder/ChatML/重複/高類似度/実在機種名/wrong-name列挙/
Phase4T〜4ZGの全既存probe・教師とのcontamination(Phase4ZG自身の新規教師・
新規held-outも含む)を確認する。multi-turn(turns形式)レコードは全ターンを検査対象にする。
...
from phase4zh_structural_hardening_source_data import (  # noqa: E402
...
def to_riru_record(item: dict, category: str) -> dict:
    ...
    return {
        "messages": messages,
        "metadata": {"source": "phase4zh_structural_hardening", "category": category,
...
def main() -> int:
    ...
    category_map = {
        "ZH-A": "assertion_resistance",
        "ZH-B": "false_memory_resistance",
        "ZH-C": "authority_metadata_spoof_resistance",
        "ZH-D": "persistent_correction_multiturn",
        "ZH-E": "instruction_override_resistance",
        "ZH-F": "role_name_separation",
        "ZH-G": "quotation_mention_safety",
        "ZH-H": "nickname_distinction",
        "ZH-I": "intrusion_control",
    }
    new_records = []
    for item in ALL_SINGLE_TURN_RECORDS + ALL_MULTITURN_RECORDS:
        prefix = item["id"][:4]
        cat = category_map.get(prefix, "structural_hardening_other")
    ...
    (REPORTS_DIR / "phase4zh_dataset_leakage_analysis.json").write_text(
    ...
    intrusion_new = sum(1 for item in ALL_SINGLE_TURN_RECORDS + ALL_MULTITURN_RECORDS if item["id"].startswith("ZH-I"))
    multiturn_new = len(ALL_MULTITURN_RECORDS)
    n_new = len(new_records)
    intrusion_ratio_of_new = round(100 * intrusion_new / n_new, 1)
    ...
    print(f"intrusion-control among new: {intrusion_new}/{n_new} ({intrusion_ratio_of_new}%)")
    print(f"multi-turn among new: {multiturn_new}/{n_new}")

    write_jsonl(PROCESSED_DIR / "riru_phase4zh_structural_hardened_candidate.jsonl", combined)
    ...
    write_jsonl(PROCESSED_DIR / "riru_phase4zh_structural_hardened_train.jsonl", train)
    write_jsonl(PROCESSED_DIR / "riru_phase4zh_structural_hardened_val.jsonl", val)
    ...
    summary = {
        ...
        "new_by_category": {
            cat: sum(1 for item in ALL_SINGLE_TURN_RECORDS + ALL_MULTITURN_RECORDS if item["id"].startswith(prefix))
    ...
    (REPORTS_DIR / "phase4zh_training_data_analysis.json").write_text(
