"""Phase 4ZE: 既存Final Candidate(ratio-high-identity-stable, 1095件)にIdentity Margin
Reinforcement教師(50件)を追加した phase4ze_identity_margin candidate を構築する。

既存1095件は無改変・読み取り専用で読み込み、新規50件のみを追加した別ファイルとして
新candidateを構築する。既存riru_ratio_high_identity_stable_candidate.jsonl等は無改変。

品質検査: placeholder/ChatML/重複/高類似度/実在機種名/wrong-name列挙/
Phase4T〜4ZDの全probe文面・新規held-out probe(phase4ze_holdout_probes.py)との
contaminationを確認する。
...
from phase4ze_identity_margin_source_data import ALL_RECORDS  # noqa: E402
from phase4x_placeholder_detector import classify_placeholder  # noqa: E402

EXISTING_CANDIDATE = PROCESSED_DIR / "riru_ratio_high_identity_stable_candidate.jsonl"

CHATML_PATTERN = re.compile(r"<\|im_start\|>|<\|im_end\|>|\[INST\]|\[/INST\]")
REAL_MACHINE_HINTS = ["ミリオンゴッド", "モンキーターン", "北斗", "ジャグラー", "バジリスク"]
# 注意: WRONG_NAME_ENUMERATIONは「1レコード内に複数の誤名が列挙されていないか」を検査する
# ためのリストであり、Category C(1レコード1誤名の訂正)自体を禁止するものではない。
WRONG_NAME_LIST = [
    "ルリ", "ルナ", "リリ", "リコ", "ルカ", "パチ子", "パチスロ君",
    "パチスロナビ", "パチスロAI", "あいこ", "ルル", "アリス",
]
...
def to_riru_record(item: dict, category: str) -> dict:
    return {
        ...
        "metadata": {"source": "phase4ze_identity_margin", "category": category,
                      "index": item["id"]},
    }
...
def quality_check(new_records: list[dict]) -> dict:
    ...
    for rec in new_records:
        ...
        # 1レコード内に複数の誤名が同時に列挙されていないかを検査(単発の訂正は許容)
            ...
            for t in texts:
                if t and (t in user_text or user_text in t):
                    contamination_hits.append(
                        {"id": rec["metadata"]["index"], "source": src_name,
                         "matched_text": t[:60]}
                    )
...
def main() -> int:
    ...
    category_map = {
        "ZE-A": "identity_direct",
        "ZE-B": "identity_greeting",
        "ZE-C": "identity_correction",
        "ZE-D": "identity_intrusion_control",
        "ZE-E": "identity_role_distinction",
    }
    new_records = []
    for item in ALL_RECORDS:
        prefix = item["id"][:4]
        cat = category_map.get(prefix, "identity_margin_other")
    ...
    (REPORTS_DIR / "phase4ze_dataset_quality.json").write_text(
    ...
    intrusion_control_new = sum(1 for item in ALL_RECORDS if item["id"].startswith("ZE-D"))
    intrusion_control_ratio_of_new = round(100 * intrusion_control_new / len(new_records), 1)
    ...
    print(f"intrusion-control among new: {intrusion_control_new}/{len(new_records)} "
          f"({intrusion_control_ratio_of_new}%)")

    if complex_ratio < 10.0:
        # Phase4Xが独自に設けていた「complex比率10%以上」という目安は、Phase4ZE自身の
        # 指示書には明記されていない社内的なsanity checkであり、新規50件のidentity教師を
        # 追加すれば分母が増える以上、数学的に必然的にわずかに低下する(113/1145=9.87%、
        # 目安からの乖離はわずか0.13pt)。Phase4ZEの明示的な禁止事項は「complex教師の追加」
        # であって「既存complex教師の比率を厳密に10%以上に保つこと」ではないため、本フェーズは
        # これをハード停止条件とはせず、警告として記録した上で続行する。
        print(f"WARNING: complex ratio {complex_ratio}% is below the Phase4X-era 10% guideline "
              f"(deviation {round(10.0 - complex_ratio, 2)}pt). Not a Phase4ZE hard requirement; "
              f"proceeding with disclosure in the report.")

    write_jsonl(PROCESSED_DIR / "riru_phase4ze_identity_margin_candidate.jsonl", combined)
    ...
    write_jsonl(PROCESSED_DIR / "riru_phase4ze_identity_margin_train.jsonl", train)
    write_jsonl(PROCESSED_DIR / "riru_phase4ze_identity_margin_val.jsonl", val)

    summary = {
        "existing_count": len(existing),
        "existing_complex_count": n_complex_existing,
        "existing_identity_count": n_identity_existing,
        ...
        "identity_total_count": identity_total,
        "identity_ratio_pct": identity_ratio,
        "intrusion_control_new_count": intrusion_control_new,
        "intrusion_control_ratio_of_new_pct": intrusion_control_ratio_of_new,
        "train_count": len(train),
        "val_count": len(val),
        "train_val_overlap": overlap,
    }
    (REPORTS_DIR / "phase4ze_dataset_summary.json").write_text(
