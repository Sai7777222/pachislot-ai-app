"""Phase 4Y Section11: Final Candidate adapter(ratio-high-identity-stable)を
Base Qwen2.5-14B-InstructへLoRA mergeする。

CPU上でbf16のまま処理する(GPU VRAM 32GBに対しbf16非量子化14Bモデルは
約29.4GBを要し、他プロセスとの余裕が乏しいため)。既存のBaseモデル・adapter・
他のcandidate/adapterは一切変更・上書きしない。merge先は新規ディレクトリとする。
"""
...
MERGED_OUTPUT_DIR = TRAINING_ROOT / "merged" / "riru-qwen-final-hf"
...
def main() -> int:
    ...
    (TRAINING_ROOT / "reports" / "phase4y_merge_run_info.json").write_text(
