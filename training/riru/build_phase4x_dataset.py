"""Phase 4X: ratio-high-identity candidateにIdentity Stabilization教師を
追加した ratio_high_identity_stable candidateを構築する。

既存 riru_ratio_high_identity_candidate.jsonl (1070件 = complex113 +
Phase4U identity教師43) は無改変・読み取り専用で読み込み、Phase4X新規
identity stabilization教師(25件)を追加した別ファイルとして新candidateを
構築する。complex教師113件・Phase4U identity教師43件は一切削除・変更しない。

品質検査: placeholder/ChatML/重複/高類似度/実在機種名/wrong-name列挙/
Phase4T〜4W全probe文面とのcontaminationを確認する。
...
from phase4x_identity_stabilization_source_data import ALL_RECORDS  # noqa: E402
from phase4x_placeholder_detector import classify_placeholder  # noqa: E402

EXISTING_CANDIDATE = PROCESSED_DIR / "riru_ratio_high_identity_candidate.jsonl"

CHATML_PATTERN = re.compile(r"<\|im_start\|>|<\|im_end\|>|\[INST\]|\[/INST\]")
REAL_MACHINE_HINTS = ["ミリオンゴッド", "モンキーターン", "北斗", "ジャグラー", "バジリスク"]
WRONG_NAME_ENUMERATION = [
    "リコ", "リサ", "アリス", "パチ子", "キリコ", "リリ", "リナ",
    "パチスロ博士", "パチスロ君", "パチスロちゃんねる", "パチスロちゃん",
    "キリエ", "あいこ", "ルリ", "ルナ", "ルル",
]
...
def to_riru_record(item: dict, category: str) -> dict:
    return {
        ...
        "metadata": {"source": "phase4x_identity_stabilization", "category": category,
                      "index": item["id"]},
    }
...
def main() -> int:
    ...
    n_identity_existing = sum(
        1 for r in existing if r["metadata"].get("source") == "phase4u_identity"
    )

    category_map = {
        "IX-A": "identity_explicit",
        "IX-B": "identity_ambiguous",
        "IX-C": "identity_casual_greeting",
        "IX-D": "identity_intrusion_control",
    }
    new_records = []
    for item in ALL_RECORDS:
        prefix = item["id"][:4]
        cat = category_map.get(prefix, "identity_stabilization_other")
    ...
    (REPORTS_DIR / "phase4x_dataset_quality.json").write_text(
    ...
    intrusion_control_new = sum(1 for item in ALL_RECORDS if item["id"].startswith("IX-D"))
    ...
    write_jsonl(PROCESSED_DIR / "riru_ratio_high_identity_stable_candidate.jsonl", combined)

    val_ratio = 106 / 1070  # ratio-high-identityのval比率を踏襲
    ...
    write_jsonl(PROCESSED_DIR / "riru_ratio_high_identity_stable_train.jsonl", train)
    write_jsonl(PROCESSED_DIR / "riru_ratio_high_identity_stable_val.jsonl", val)
    ...
    (REPORTS_DIR / "phase4x_dataset_summary.json").write_text(
