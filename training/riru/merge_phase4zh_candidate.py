"""Phase 4ZH Section27-28: Phase4ZH candidate adapter(lora-riru-qwen-phase4zh-structural-hardened)を
Base Qwen2.5-14B-InstructへLoRA mergeする。

HF Gate(Section26)が完全PASSした場合のみ実行する。CPU上でbf16のまま処理する
(Phase4Y/4ZE/4ZGと同一の安全な方式)。既存のBaseモデル・adapter・既存merged HF
(riru-qwen-final-hf, riru-phase4ze-identity-margin-hf)・他のcandidate/adapterは
...
MERGED_OUTPUT_DIR = TRAINING_ROOT / "merged" / "riru-phase4zh-structural-hardened-hf"
...
def main() -> int:
    ...
    (TRAINING_ROOT / "reports" / "phase4zh_merge_run_info.json").write_text(
