"""Phase 4ZE Section21: Phase4ZE candidate adapter(lora-riru-qwen-phase4ze-identity-margin)を
Base Qwen2.5-14B-InstructへLoRA mergeする。

CPU上でbf16のまま処理する(Phase4Yと同一の安全な方式)。既存のBaseモデル・adapter・
既存merged HF(riru-qwen-final-hf)・他のcandidate/adapterは一切変更・上書きしない。
merge先は新規ディレクトリとする。
"""
...
MERGED_OUTPUT_DIR = TRAINING_ROOT / "merged" / "riru-phase4ze-identity-margin-hf"
...
def main() -> int:
    ...
    (TRAINING_ROOT / "reports" / "phase4ze_merge_run_info.json").write_text(
