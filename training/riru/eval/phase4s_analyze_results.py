"""Phase 4S: 包括評価結果の集約分析 (Base/v4/ratio_mid/ratio_high)。"""
...
def main() -> int:
    data = json.loads((EVAL_DIR / "phase4s_comprehensive_results.json").read_text(encoding="utf-8"))
    ...
    criteria = {}
    for cond in ("C_mid", "D_high"):
        s = summary[cond]
        crit = {
            "q3_recall_ge80": s["q3_sampled_avg_recall"] >= 80.0,
            "q3_all3_gamecount_ge4of5": s["q3_all3_gamecount_seeds"] >= 4,
            "q3_all3_pct_ge4of5": s["q3_all3_pct_seeds"] >= 4,
            "p01_ge80": s["p01_avg_recall"] >= 80.0,
            "p02_ge80": s["p02_avg_recall"] >= 80.0,
            "p04_ge70_and_not_worse_than_v4": (
                s["p04_avg_recall"] >= 70.0
                and s["p04_avg_recall"] >= summary["B_v4"]["p04_avg_recall"] - 10.0
            ),
            "q9_clean": s["q9_calc_hallucination_seeds"] == 0,
            "q11_yamedoki_clean": s["q11_yamedoki_seeds"] == 0,
            "q11_strategy_clean": s["q11_strategy_seeds"] == 0,
            "q11_causal_clean": (
    ...
    out_path = REPORTS_DIR / "phase4s_ratio_performance_analysis.json"
